...
import { Rx } from "./project/context";

export async function change(
  filenameOrFilepath: MaybePromise<string>,
  request: string,
  ...
  if (await fileExists(filenameOrFilepath)) {
    ...
    return _change(
      await getFilepath(filenameOrFilepath),
      request,
      projectDirectory
    );
  }
  ...
  const response = await sequence([
    g4(
      system(`You are an expert programmer. Make the requested changes to the file provided.
            ${await loadProjectStyleGuide(
              new Rx({ projectDirectory, name: "adhoc" }).spawn()
            )}`),
      ...
      user(request),
      system(
        `Write one to two sentences to show your understanding of the issue.`
      )
    ),
    g4(
      system(`
  ...
  await askToAcceptProposal(filepath, {
    onContinue: async () =>
      await rewriteChange(
        filepath,
        request,
        await getInput("Feedback on this change? ")
      ),
  });
}

export async function rewriteChange(
  filename: string,
  originalChangeInstructions: string,
  feedback: string
) {
  ...
  const response = await chat(
    g35(
      system(
        `The user asked for a change to be made to the file ${filename}. The AI proposed a change and the user has feedback on that change. update the change as the user asks.`
      ),
  ...
  await askToAcceptProposal(filename, {
    onContinue: async () =>
      await rewriteChange(
        filename,
        originalChangeInstructions + " // " + feedback,
        await getInput("Feedback on this change? ")
      ),
  });
}

export async function saveCodeSnippetAsProposal(
  filename: string,

...
export function g35_t02(messages: MessageOrStr[]): ChatSpec;
export function g35_t02(...messages: MessageOrStr[]): ChatSpec;
export function g35_t02(
  ...messages: MessageOrStr[] | [MessageOrStr[]]
): ChatSpec {
  const flattenedMessages = (
    Array.isArray(messages[0]) ? messages[0] : messages
...
export async function readAndLogStream(
...
export function estimatePricing({
  spec: { model, messages: inputMessages },
  output,
}: {
  spec: ChatSpec;
  output?: string;
  ...
  const inputPrice =
    numTokens(model, inputMessages) * inputPricePerToken[model];
  const outputPrice =
    numTokens(model, output ?? "") * outputPricePerToken[model];
...
export function assistant_dedent(
  strings: TemplateStringsArray,
  ...values: any[]
): Message;

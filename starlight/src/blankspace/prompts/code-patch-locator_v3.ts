...
Your task is to analyze the original source code file and a provided snippet of code intended to update the file. You need to determine which section of the original file should be replaced with this patch. Your response should be the line numbers of the first and last lines to be replaced, in the format: { startingLine, endingLine }. 

The original source code file and the snippet of code will be provided in the subsequent user messages.
...
export type Prompt = {
  spec: typeof spec;
  filename: "code-patch-locator_v3.ts";
  inferred: {
    inputs: string[];
    returns: {
      startingLine: number;

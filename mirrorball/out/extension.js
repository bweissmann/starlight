...
exports.deactivate = exports.BlankspaceActionProvider = exports.activate = void 0;
...
function activate(context) {
    ...
    context.subscriptions.push(vscode.languages.registerCodeActionsProvider('typescript', new BlankspaceActionProvider(), {
        providedCodeActionKinds: BlankspaceActionProvider.providedCodeActionKinds
    }));
}
exports.activate = activate;
function generate(call) {
    (0, redis_1.getRedisClient)().then(client => {
        console.log("Sending to redis");
        if (!client) {
            console.error("Could not connect to Redis, unable to generate prompt");
            return;
        }
        client.xAdd('mirrorball', '*', { "type": "request", "uuid": (0, uuid_1.v4)(), "input": stripQuotes(call.getArguments()[0].getText()) });
    });
}
...
BlankspaceActionProvider.providedCodeActionKinds = [
    vscode.CodeActionKind.QuickFix
];
function findCallExpression(filePath, range) {
    try {
        ...
        if (node) {
            const callExpression = node.getFirstAncestorByKind(ts_morph_1.SyntaxKind.CallExpression);
